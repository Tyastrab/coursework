...
```{r graph-data}
ggplot(trips_per_day, aes(x = tmin, y = num_trips, color = weekday)) +
  geom_point() +
...
for (k in K) {
    ...
    model <- lm(num_trips ~ poly(tmin, k, raw = T) - count, data=trips_per_day_train)
...
```{r }
model <- lm(num_trips ~ poly(tmin, 4, raw = T) - count, data = trips_per_day_train)
